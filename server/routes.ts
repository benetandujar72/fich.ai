...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/communications", isAuthenticated, async (req, res) => {
    try {
      ...
      const communications = await storage.getCommunications(userId, filter);
  ...
  // Get users for communication recipients
  app.get("/api/users/:institutionId", isAuthenticated, async (req, res) => {
    try {
      const { institutionId } = req.params;
